...
// Source: https://github.com/hughsk/glsl-hsv2rgb
vec3 hsv2rgb(vec3 c) {
...
void main(void) {
	...
	float ratio_wh = float(gl_NumWorkGroups.x) / float(gl_NumWorkGroups.y);

	vec2 uv = vec2(
		float(gl_WorkGroupID.x) / float(gl_NumWorkGroups.x),
		float(gl_WorkGroupID.y) / float(gl_NumWorkGroups.y));
	...
	// color = vec4(noise);
	color = vec4(hsv2rgb(vec3(noise, 0.7, 1.0)), 0.0);
