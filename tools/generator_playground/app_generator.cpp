...
void app_t::loop_generator() {
	...
	if (global_settings.draw_player and
			generator_C.tour_path_points_generated()) {
		mat4 MVP(1);
		...
		world_pos.x /= generator_C.get_space_max().x*3.0;
		world_pos.y /= generator_C.get_space_max().y;
		world_pos.y = 1.0 - world_pos.y;
		world_pos *= 2.0;
		world_pos.x -= 1.0;
		world_pos.y -= 1.0;
		world_pos.y
			= world_pos.y * generator_C.get_ratio_hw();
